...
]).directive('uiSplitbar', function ($rootScope) {
    // Get all the page.
    var htmlElement = angular.element(document.body.parentElement);
    return {
        require: '^uiLayout',
        restrict: 'EAC',
        link: function (scope, iElement, iAttrs, parentLayout, abc) {
            var animationFrameRequested, lastX, lastForcedSize;
            var _cache = {};
            // Use relative mouse position
            var isUsingColumnFlow = iAttrs.$attr.flow === 'column';
            ...
            function _cached_layout_values() {
                ...
                _cache.previousElement.name = $(previousElement).data('uiLayoutId');
                ...
                _cache.nextElement.name = $(nextElement).data('uiLayoutId');
            ...
            function _draw() {
                ...
                if (forcedSize) the_pos = forcedSize;
            ...
            function _resize(mouseEvent) {
                ...
                lastForcedSize = (lastX - _cache.layoutOrigine) / _cache.layoutSize * 100;
            }
            var forcedSize = 0;
            ...
            var self = this;
            $rootScope.$on('uiLayout::resizePane', function(event, options) {
                _cached_layout_values();

                var prevName = _cache.previousElement.name;
                var nextName = _cache.nextElement.name;
                if (!prevName || !nextName) return;

                var startingPos;
                if (!lastForcedSize) startingPos = _cache.layoutOrigine;
                else startingPos = lastForcedSize;

                var sizeHolder = {
                    forcedSize: startingPos
                };
                var finalSize = options.size;
                lastForcedSize = finalSize;
                $(sizeHolder).animate({forcedSize: finalSize}, {
                    duration: 400,
                    ease: 'easeOutElastic',
                    step: function(now) {
                        forcedSize = now;
